...
cin: ['', [
    ...
    this.expertForm.get('gouvernat')?.valueChanges.subscribe(gouvernat => {
      this.villes = this.villesParGouvernorat[gouvernat] || [];
      this.expertForm.get('ville')?.reset();
      this.expertForm.get('codePostal')?.reset();
    });

    // Watch for ville changes to update codePostal
    this.expertForm.get('ville')?.valueChanges.subscribe(ville => {
      const codePostal = this.codesPostauxParVille[ville] || '';
      this.expertForm.get('codePostal')?.setValue(codePostal);
    });
  }
  ...
  onSubmit(): void {
    ...
    const userData = {
      ...
      adresse: {
        ...
        codePostal:Number(this.expertForm.value.codePostal),
        gouvernat: this.expertForm.value.gouvernat,
        pays: 'Tunisie',
      },
    };
    ...
    this.http.post('http://localhost:3000/auth/register', userData).subscribe(
      (userResponse: any) => {
        ...
        this.http.post('http://localhost:3000/expert/addExpert', expertData).subscribe(
          (expertResponse: any) => {
            this.router.navigate(['/admin/listExpert']);
          },
          (expertError) => {
      ...
      (userError) => {
        window.alert("Vérifiez vos informations");
