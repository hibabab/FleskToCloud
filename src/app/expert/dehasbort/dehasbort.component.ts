...
import { UserDto } from '../../assure/models/user-dto'; // Ajustez le chemin selon votre structure
import { UserService } from '../../assure/services/user-service.service'; // Ajustez le chemin selon votre structure
// Ajustez le chemin selon votre structure
import { NotificationService } from '../../agent-service/Services/notification.service'; // Ajustez le chemin selon votre structure
...
export class DehasbortExpertComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (token) {
      try {
        ...
        this.router.navigate(['/espace-client/login']); // Redirection en cas d'erreur
      }
    } else {
      console.error('Token JWT non trouvé');
      this.router.navigate(['/espace-client/login']); // Rediriger vers la page de connexion si pas de token
    }
  }
